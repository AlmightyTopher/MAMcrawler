...
class ProwlarrCuratedSearch:
    ...
    def search_prowlarr_for_book(self, book: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                print(f"  No results (HTTP {response.status_code})")
                return results

            data = response.json()
            print(f"  Prowlarr returned {len(data)} total results")
    ...
    def _is_book_match(self, book: Dict[str, Any], found_title: str) -> bool:
        ...
        # Must contain audiobook or audio indicators (relaxed)
        audio_indicators = ['audiobook', 'audio', 'mp3', 'm4b', 'aac', 'book', 'novel']
        has_audio = any(indicator in search_lower for indicator in audio_indicators)

        # Must contain title keywords (relaxed)
        title_words = title_lower.split()
        title_matches = sum(1 for word in title_words if word in search_lower)

        # Must contain author keywords (relaxed)
        author_words = author_lower.split()
        author_matches = sum(1 for word in author_words if word in search_lower)

        # Reasonable match criteria - relaxed for better results
        is_match = has_audio and (title_matches >= 1 or author_matches >= 1)

        # Debug output for non-matches
        if not is_match and len(results) < 3:  # Only show first few rejections
            print(f"  Skipping: {found_title} (audio: {has_audio}, title: {title_matches}, author: {author_matches})")
    ...
    def save_report(self):
        ...
        try:
            with open(report_file, 'w', encoding='utf-8') as f:
                json.dump(self.results, f, indent=2, ensure_ascii=False, fp=f)
