# qBittorrent WebUI API Documentation (v5.1.3)
...
- [RSS (Experimental)](#rss-experimental)
- [Sync](#sync)
- [Search](#search)
- [Log](#log)
- [Torrent Creator](#torrent-creator)
- [WebAPI Versioning](#webapi-versioning)

## General Information
All API methods follow the format `/api/v2/APIName/methodName`, where `APIName` is a certain subgroup of API methods whose functionality is related.

All API methods only allow GET or POST methods. Use POST when you are mutating some state (or when your request is too big to fit into GET) and use GET otherwise. Starting with qBittorrent v4.4.4, server will return 405 Method Not Allowed when you used the wrong request method.
...
**Response:**
- HTTP 403: User's IP is banned for too many failed login attempts
- HTTP 200: All other scenarios
...
**Response:** String with application version (e.g. v5.1.3)

### Get API version
**Endpoint:** `GET /api/v2/app/webapiVersion`

**Response:** String with WebAPI version (e.g. 2.11)

### Get build info
**Endpoint:** `GET /api/v2/app/buildInfo`

**Response:** JSON object with build information:
- `qt`: QT version
- `libtorrent`: libtorrent version
- `boost`: Boost version
- `openssl`: OpenSSL version
- `zlib`: zlib version
- `bitness`: Application bitness (e.g. 64-bit)

### Shutdown application
**Endpoint:** `GET /api/v2/app/shutdown`
...
**Endpoint:** `PATCH /api/v2/app/preferences`

**Parameters:** JSON object with key-value pairs of settings to change

### Get default save path
**Endpoint:** `GET /api/v2/app/defaultSavePath`

**Response:** String with default save path

### Get cookies
**Endpoint:** `GET /api/v2/app/cookies`

**Response:** JSON array of cookie objects

### Set cookies
**Endpoint:** `POST /api/v2/app/setCookies`

**Parameters:** JSON array of cookie objects
...
- `dl_rate_limit`: Download rate limit (bytes/s)
- `up_rate_limit`: Upload rate limit (bytes/s)
- `dht_nodes`: DHT nodes connected to
- `connection_status`: Connection status (connected/firewalled/disconnected)

### Get alternative speed limits state
**Endpoint:** `GET /api/v2/transfer/speedLimitsMode`

**Response:** 1 if alternative speed limits enabled, 0 otherwise
...
**Response:** Current global download speed limit in bytes/second

### Set global download limit
**Endpoint:** `POST /api/v2/transfer/setDownloadLimit`

**Parameters:**
- `limit` (integer): Download speed limit in bytes/second

### Get global upload limit
**Endpoint:** `GET /api/v2/transfer/uploadLimit`

**Response:** Current global upload speed limit in bytes/second

### Set global upload limit
**Endpoint:** `POST /api/v2/transfer/setUploadLimit`

**Parameters:**
- `limit` (integer): Upload speed limit in bytes/second

### Ban peers
**Endpoint:** `POST /api/v2/transfer/banPeers`

**Parameters:**
- `peers` (string): Peer addresses separated by | (host:port format)
...
**Optional Parameters:**
- `filter`: Filter by state (all, downloading, seeding, completed, stopped, active, inactive, running, stalled, stalled_uploading, stalled_downloading, errored)
...
**Response:** JSON object with detailed torrent properties

### Get torrent trackers
**Endpoint:** `GET /api/v2/torrents/trackers`

**Required Parameters:**
- `hash`: Torrent hash

**Response:** JSON array of tracker objects

### Get torrent web seeds
**Endpoint:** `GET /api/v2/torrents/webseeds`

**Required Parameters:**
- `hash`: Torrent hash

**Response:** JSON array of web seed URLs
...
**Optional Parameters:**
- `indexes`: File indexes to retrieve

**Response:** JSON array of file objects

### Get torrent pieces' states
**Endpoint:** `GET /api/v2/torrents/pieceStates`

**Required Parameters:**
- `hash`: Torrent hash

**Response:** Array of piece states (0=not downloaded, 1=downloading, 2=downloaded)

### Get torrent pieces' hashes
**Endpoint:** `GET /api/v2/torrents/pieceHashes`

**Required Parameters:**
- `hash`: Torrent hash

**Response:** Array of piece hashes

### Pause torrents
**Endpoint:** `POST /api/v2/torrents/pause`

**Optional Parameters:**
- `hashes`: Torrent hashes (separated by |, or "all")
...
- `urls`: URLs separated by newlines
- `torrents`: Raw torrent file data

**Optional Parameters:**
- `savepath`: Download folder
- `category`: Category for torrent
- `tags`: Tags for torrent
- `skip_checking`: Skip hash checking
- `paused`: Add in paused state
- `root_folder`: Create root folder
- `rename`: Rename torrent
- `upLimit`: Upload speed limit
- `dlLimit`: Download speed limit
...
**Endpoint:** `POST /api/v2/torrents/addTrackers`

**Required Parameters:**
- `hash`: Torrent hash
- `urls`: Tracker URLs separated by newlines

### Edit trackers
**Endpoint:** `POST /api/v2/torrents/editTracker`

**Required Parameters:**
- `hash`: Torrent hash
- `origUrl`: Original tracker URL
- `newUrl`: New tracker URL

### Remove trackers
**Endpoint:** `POST /api/v2/torrents/removeTrackers`
...
### Add peers
**Endpoint:** `POST /api/v2/torrents/addPeers`

**Required Parameters:**
- `hashes`: Torrent hashes (separated by |)
- `peers`: Peer addresses (host:port, separated by |)

### Set torrent category
**Endpoint:** `POST /api/v2/torrents/setCategory`
...
**Response:** JSON object of categories
...
**Optional Parameters:**
- `savePath`: Save path for category

### Edit category
**Endpoint:** `POST /api/v2/torrents/editCategory`
...
### Remove categories
**Endpoint:** `POST /api/v2/torrents/removeCategories`

**Required Parameters:**
- `categories`: Category names separated by newlines

### Add torrent tags
**Endpoint:** `POST /api/v2/torrents/addTags`
...
### Remove torrent tags
**Endpoint:** `POST /api/v2/torrents/removeTags`

**Required Parameters:**
- `hashes`: Torrent hashes (separated by |, or "all")
- `tags`: Tags to remove (comma-separated, empty removes all)

### Get all tags
**Endpoint:** `GET /api/v2/torrents/tags`

**Response:** JSON array of tag strings

### Create tags
**Endpoint:** `POST /api/v2/torrents/createTags`

**Required Parameters:**
- `tags`: Tags to create (comma-separated)

### Delete tags
**Endpoint:** `POST /api/v2/torrents/deleteTags`

**Required Parameters:**
- `tags`: Tags to delete (comma-separated)

### Set automatic torrent management
**Endpoint:** `POST /api/v2/torrents/setAutoManagement`
...
**Endpoint:** `POST /api/v2/torrents/toggleSequentialDownload`

**Optional Parameters:**
- `hashes`: Torrent hashes (separated by |, or "all")

### Set first/last piece priority
**Endpoint:** `POST /api/v2/torrents/toggleFirstLastPiecePrio`

**Optional Parameters:**
- `hashes`: Torrent hashes (separated by |, or "all")

### Set force start
**Endpoint:** `POST /api/v2/torrents/setForceStart`
...
### Set super seeding
**Endpoint:** `POST /api/v2/torrents/setSuperSeeding`
...
### Rename file
**Endpoint:** `POST /api/v2/torrents/renameFile`
...
### Rename folder
**Endpoint:** `POST /api/v2/torrents/renameFolder`
...
### Export torrent
**Endpoint:** `GET /api/v2/torrents/export`

**Required Parameters:**
- `hash`: Torrent hash

**Response:** Torrent file binary

## RSS (Experimental)
All RSS API methods are under "rss", e.g.: `/api/v2/rss/methodName`.

### Add folder
**Endpoint:** `POST /api/v2/rss/addFolder`

**Required Parameters:**
- `path`: Full path of added folder

### Add feed
**Endpoint:** `POST /api/v2/rss/addFeed`
...
### Remove item
**Endpoint:** `POST /api/v2/rss/removeItem`

**Required Parameters:**
- `path`: Full path of item to remove

### Move item
**Endpoint:** `POST /api/v2/rss/moveItem`

**Required Parameters:**
- `itemPath`: Current full path
- `destPath`: New full path

### Get all items
**Endpoint:** `GET /api/v2/rss/items`

**Optional Parameters:**
- `withData`: Include current feed articles

**Response:** JSON RSS structure

### Mark as read
**Endpoint:** `POST /api/v2/rss/markAsRead`

**Required Parameters:**
- `itemPath`: Full path of item

**Optional Parameters:**
- `articleId`: Specific article ID

### Refresh item
**Endpoint:** `POST /api/v2/rss/refreshItem`

**Required Parameters:**
- `itemPath`: Full path of item

### Set auto-downloading rule
**Endpoint:** `POST /api/v2/rss/setRule`

**Required Parameters:**
- `ruleName`: Rule name
- `ruleDef`: JSON rule definition

### Rename auto-downloading rule
**Endpoint:** `POST /api/v2/rss/renameRule`

**Required Parameters:**
- `ruleName`: Current rule name
- `newRuleName`: New rule name

### Remove auto-downloading rule
**Endpoint:** `POST /api/v2/rss/removeRule`

**Required Parameters:**
- `ruleName`: Rule name

### Get all auto-downloading rules
**Endpoint:** `GET /api/v2/rss/rules`

**Response:** JSON object of rules

### Get all articles matching a rule
**Endpoint:** `GET /api/v2/rss/matchingArticles`

**Required Parameters:**
- `ruleName`: Rule name

**Response:** JSON object of matching articles
...
**Response:** JSON with sync data and response ID

### Get torrent peers data
**Endpoint:** `GET /api/v2/sync/torrentPeers`
...
**Response:** JSON with peers data and response ID

## Search
All Search API methods are under "search", e.g.: `/api/v2/search/methodName`.

### Start search
**Endpoint:** `POST /api/v2/search/start`

**Required Parameters:**
- `pattern`: Search pattern
- `category`: Categories to search
- `plugins`: Search plugins to use

**Response:** JSON with search ID

### Stop search
**Endpoint:** `POST /api/v2/search/stop`

**Required Parameters:**
- `id`: Search job ID

### Get search status
**Endpoint:** `GET /api/v2/search/status`

**Optional Parameters:**
- `id`: Search job ID

**Response:** JSON array of search statuses
...
**Response:** JSON with results and status

### Delete search
**Endpoint:** `POST /api/v2/search/delete`

**Required Parameters:**
- `id`: Search job ID

### Get search plugins
**Endpoint:** `GET /api/v2/search/plugins`

**Response:** JSON array of plugin info

### Install search plugin
**Endpoint:** `POST /api/v2/search/installPlugin`

**Required Parameters:**
- `sources`: Plugin sources (URLs or file paths)

### Uninstall search plugin
**Endpoint:** `POST /api/v2/search/uninstallPlugin`

**Required Parameters:**
- `names`: Plugin names to uninstall

### Enable search plugin
**Endpoint:** `POST /api/v2/search/enablePlugin`
...
**Endpoint:** `POST /api/v2/search/updatePlugins`

## Log
All Log API methods are under "log", e.g.: `/api/v2/log/methodName`.

### Get log
**Endpoint:** `GET /api/v2/log/main`
...
- `last_known_id`: Exclude messages with ID <= this value

**Response:** JSON array of log entries

### Get peer log
**Endpoint:** `GET /api/v2/log/peers`

**Optional Parameters:**
- `last_known_id`: Exclude messages with ID <= this value

**Response:** JSON array of peer log entries

## Torrent Creator
All Torrent Creator API methods are under "torrents/create", e.g.: `/api/v2/torrents/create/methodName`.

### Create torrent
**Endpoint:** `POST /api/v2/torrents/create`

**Required Parameters:**
- `sourcePath`: Source path

**Optional Parameters:**
- `torrentFilePath`: Output torrent file path
...
- `startSeeding`: Start seeding after creation

**Response:** JSON with task ID

### Get torrent creation status
**Endpoint:** `GET /api/v2/torrents/create/status`

**Optional Parameters:**
- `taskID`: Specific task ID

**Response:** JSON array of task statuses

### Get created torrent file
**Endpoint:** `GET /api/v2/torrents/create/torrentFile`
...
### Delete torrent creation task
**Endpoint:** `POST /api/v2/torrents/create/delete`
...
## Additional Notes
- This documentation is based on qBittorrent v5.1.3 source code analysis
- All endpoints require authentication except `/api/v2/auth/login`
- Parameters marked as "optional" may be omitted
- Response formats are JSON unless specified otherwise
- File operations return appropriate HTTP status codes
General Information
...
https://github.com/qbittorrent/qBittorrent.wiki.git
Footer
© 2025 GitHub, Inc.
Footer navigation
Terms
